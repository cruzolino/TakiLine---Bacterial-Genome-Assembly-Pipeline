...
# abricate/mlst/busco/checkm2/canu each run in their own conda env — see README.
...
run_quality_assessment() {
    ...
    if _conda_env_tool_available "${CHECKM2_ENV}" checkm2; then
        ...
        conda run -n "${CHECKM2_ENV}" checkm2 predict \
            --input "${ASSEMBLY}" \
...
generate_report() {
    ...
    if ${PLASMID_DETECTION}; then
        ...
        if [[ -n "${pf_tab}" && -s "${pf_tab}" ]]; then
            ...
            # This PlasmidFinder version doesn't write results_tab.tsv; count hits in data.json directly.
...
" 2>/dev/null) || pf_hit_count="N/A (parse error)"
    ...
    # Built in Markdown and HTML together from the same values, so they can't drift.
